...
─────────────────────────────────────────────────
  ...
  15:00  KR Market  올업데이트 (종가베팅V2 + 수급/VCP/AI/리포트) → 텔레그램
...
class Config:
    ...
    KR_UPDATE_TIME = os.environ.get('KR_MARKET_UPDATE_TIME', '15:00')
...
def run_kr_full_update(skip_sync: bool = False):
    """KR 올업데이트 (15:00) — 종가베팅V2 + 수급/VCP/AI/리포트 → 통합 텔레그램"""
    logger.info("=" * 60)
    logger.info("🇰🇷 KR 올업데이트 시작 (15:00)")
    logger.info("=" * 60)
    ...
    logger.info(f"📋 KR 올업데이트 완료: {success_count}/{total_count} ({elapsed}초)")

    msg = (
        f"<b>🇰🇷 15시 KR 올업데이트 완료</b>\n"
...
def run_full_update():
    ...
    tasks = [
        ...
        ("KR Market",   "🇰🇷", lambda: run_kr_full_update(skip_sync=True)),
...
class Scheduler:
    ...
    def setup_schedules(self):
        ...
        logger.info(f"   🇺🇸 평일 {Config.US_UPDATE_TIME} US Market 전체 갱신 + Smart Money Top 5 텔레그램")
        logger.info(f"   🇺🇸 평일 {Config.US_TRACK_TIME} US Track Record 스냅샷")
        logger.info(f"   🇰🇷 평일 {Config.KR_UPDATE_TIME} KR 올업데이트 (종가베팅V2 + 수급/VCP/AI) 텔레그램")
        logger.info(f"   🇰🇷 토요일 {Config.HISTORY_TIME} 히스토리 수집")
        logger.info(f"   🪙 매일 {', '.join(Config.CRYPTO_TIMES)} Crypto 전체 파이프라인 텔레그램")
    ...
    def run(self):
        ...
        send_telegram(
            ...
            f"🇰🇷 KR Market: {Config.KR_UPDATE_TIME} (평일)\n"
            f"🪙 Crypto: 매 4시간 (24/7)\n"
            f"📍 {Config.BASE_DIR}"
        )
...
def main():
    ...
    parser.add_argument('--kr-update', action='store_true', help='KR 올업데이트 (종가베팅V2 + 수급/VCP/AI)')
